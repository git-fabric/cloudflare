...
export function createApp(adapterOverride?: CloudflareAdapter): FabricApp {
  const cf = adapterOverride ?? createAdapterFromEnv();

  function accountId(args: Record<string, unknown>): string {
    const id = (args.account_id as string | undefined) ?? cf.accountId;
    if (!id) throw new Error('account_id required (or set CLOUDFLARE_ACCOUNT_ID)');
    return id;
  }
  ...
  const tools: FabricTool[] = [
    { name: 'cf_list_zones', description: 'List zones (domains) in the Cloudflare account.',
      ...
      execute: async (a) => { const p: Record<string,unknown> = {}; if (a.name) p.name=a.name; if (a.status) p.status=a.status; if (a.page) p.page=a.page; if (a.per_page) p.per_page=a.per_page; return cf.get('/zones', p); } },
    ...
    { name: 'cf_list_dns_records', description: 'List DNS records for a zone.',
      ...
      execute: async (a) => { const p: Record<string,unknown> = {}; if (a.type) p.type=a.type; if (a.name) p.name=a.name; if (a.content) p.content=a.content; if (a.page) p.page=a.page; if (a.per_page) p.per_page=a.per_page; return cf.get(`/zones/${a.zone_id}/dns_records`, p); } },
    { name: 'cf_create_dns_record', description: 'Create a DNS record in a zone.',
      ...
      execute: async (a) => { const b: Record<string,unknown> = { type: a.type, name: a.name, content: a.content, ttl: a.ttl ?? 1 }; if (a.proxied !== undefined) b.proxied=a.proxied; if (a.priority) b.priority=a.priority; if (a.comment) b.comment=a.comment; return cf.post(`/zones/${a.zone_id}/dns_records`, b); } },
    { name: 'cf_update_dns_record', description: 'Update an existing DNS record.',
      ...
      execute: async (a) => { const b: Record<string,unknown> = { type: a.type, name: a.name, content: a.content }; if (a.ttl) b.ttl=a.ttl; if (a.proxied !== undefined) b.proxied=a.proxied; if (a.comment) b.comment=a.comment; return cf.put(`/zones/${a.zone_id}/dns_records/${a.record_id}`, b); } },
    ...
    { name: 'cf_purge_cache', description: 'Purge cache for a zone. Purge everything or specific files/tags/hosts.',
      ...
      execute: async (a) => { const b: Record<string,unknown> = {}; if (a.purge_everything) { b.purge_everything=true; } else { if (a.files) b.files=a.files; if (a.tags) b.tags=a.tags; if (a.hosts) b.hosts=a.hosts; } return cf.post(`/zones/${a.zone_id}/purge_cache`, b); } },
    { name: 'cf_list_kv_namespaces', description: 'List Workers KV namespaces.',
      ...
      execute: async (a) => { const p: Record<string,unknown> = {}; if (a.page) p.page=a.page; if (a.per_page) p.per_page=a.per_page; return cf.get(`/accounts/${accountId(a)}/storage/kv/namespaces`, p); } },
    { name: 'cf_list_kv_keys', description: 'List keys in a KV namespace.',
      ...
      execute: async (a) => { const p: Record<string,unknown> = {}; if (a.prefix) p.prefix=a.prefix; if (a.limit) p.limit=a.limit; if (a.cursor) p.cursor=a.cursor; return cf.get(`/accounts/${accountId(a)}/storage/kv/namespaces/${a.namespace_id}/keys`, p); } },
    ...
    { name: 'cf_write_kv_value', description: 'Write a key-value pair to KV storage.',
      ...
      execute: async (a) => { const p: Record<string,string> = {}; if (a.expiration_ttl) p.expiration_ttl = String(a.expiration_ttl); await cf.putRaw(`https://api.cloudflare.com/client/v4/accounts/${accountId(a)}/storage/kv/namespaces/${a.namespace_id}/values/${a.key}`, a.value as string, p); return { ok: true }; } },
    ...
    { name: 'cf_zone_analytics', description: 'Get analytics for a zone (requests, bandwidth, threats, pageviews).',
      ...
      execute: async (a) => { const p: Record<string,unknown> = {}; if (a.since) p.since=a.since; if (a.until) p.until=a.until; return cf.get(`/zones/${a.zone_id}/analytics/dashboard`, p); } },
  ];
  ...
  return {
    ...
    async health() {
      ...
      try { await cf.get('/zones', { per_page: '1' }); return { app: '@git-fabric/cloudflare', status: 'healthy', latencyMs: Date.now()-start }; }
      catch (e: unknown) { return { app: '@git-fabric/cloudflare', status: 'unavailable', latencyMs: Date.now()-start, details: { error: String(e) } }; }
    },
  };
}
